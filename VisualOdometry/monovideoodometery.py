...
class MonoVideoOdometery(object):
    ...
    def visual_odometery(self):
        ...
        if self.id < 2:
            ...
            if (True and (absolute_scale > 0.1 and abs(t[2][0]) > abs(t[0][0]) and abs(t[2][0]) > abs(t[1][0]))):
                print("entered the inner if")
    ...
    def get_absolute_scale(self):
        ...
        # true_vect = np.array([0, 0, 10])
        true_vect = np.array([[x], [y], [z]])
        self.true_coord = true_vect
        prev_vect = np.array([[x_prev], [y_prev], [z_prev]])
        # prev_vect = np.array([0, 0, 0])
